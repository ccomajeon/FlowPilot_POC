...
개인 할 일 관리와 인증 사용자 공용 게시판을 제공하는 REST API입니다. Java 21, Spring Boot, PostgreSQL을 사용합니다.
...
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.boards TO todo_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.board_posts TO todo_app;
```
...
SELECT has_table_privilege('todo_app', table_name, privilege)
FROM unnest(ARRAY['public.boards', 'public.board_posts']) AS table_name,
...
- `GET /api/v1/boards`, `GET /api/v1/boards/{boardId}`
- `POST /api/v1/boards`, `PATCH /api/v1/boards/{boardId}`
- `GET|POST /api/v1/boards/{boardId}/posts`
- `GET|PATCH|DELETE /api/v1/posts/{postId}`

메뉴와 게시물 변경에는 상세 응답의 strong ETag를 `If-Match`로 보내야 합니다. 누락은 `428`, 오래된 버전은 `409`입니다. 비활성 메뉴는 일반 조회와 게시물 생성·변경에서 차단되며 데이터는 보존됩니다. 게시물 DELETE도 행을 제거하지 않고 일반 조회에서 즉시 제외합니다.
...
The runtime account must only have `SELECT`, `INSERT`, `UPDATE`, and `DELETE` privileges on `todos`, `boards`, and `board_posts`. Use the separate Flyway account for DDL. Todo endpoints require the `todos` scope, board endpoints use `boards` or `boards.admin` as documented above, and protected Actuator endpoints require `todos.admin`.
...
3. 신규 스키마에서 이전 애플리케이션 이미지를 기동해 Todo 읽기·쓰기 회귀 테스트를 수행하고 게시판 행 수와 표본 checksum이 유지되는지 확인합니다.
...
게시판 장애 시 `boards` scope 발급을 중단하고 메뉴를 비활성화한 뒤 호환 가능한 이전 이미지를 재배포합니다. V2 테이블과 논리 삭제 데이터는 롤백 과정에서 삭제하지 않습니다. 게시판 공개 전에는 운영 PostgreSQL 버전, 사용자별 쓰기 rate limit, 콘텐츠 운영 담당자, 안전 렌더링, SLO/RPO/RTO 및 백업/PITR 복원 결과를 승인해야 합니다.
