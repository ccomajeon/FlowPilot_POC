...
Docker is mandatory for tests. If Testcontainers cannot start PostgreSQL, the test build fails instead of skipping integration tests. Preserve Surefire XML reports and Testcontainers logs in CI.
...
Readiness includes the database health indicator. Before production deployment, rehearse backup restoration and verify Flyway, constraints, row counts, and compatibility with the previous application image. Do not edit an applied migration or drop production tables as an application rollback mechanism.
